...
for i=1:15
q(i)=calc_q_PL(xin,gammad(i),efficiency_tmp(i));
%fmax(i)=1;
j=1;
   % while ceil(fmax(i))~=0 || j<100
   while j<5 
   [amin_k(i),r1(i),r2(i)]=vec_f_g_PL_cvx(q(i),gammad(i),efficiency_tmp(i));
    %fmax(i)=-bmin_k(i);
    q(i)=calc_q_PL(amin_k(i),gammad(i),efficiency_tmp(i)); 
    j=j+1;
    end
end
