...
function output=Path_loss_micro_str(x,y)
...
%  check if the transmission antenna is located in the street
...
for l = 1:length(j)    
    ...
    if (x_p==x_bs) && (y_p==y_bs)
        loss_outdoor(j(l),k(l),1)=NaN;
        continue;
    end
...
% Rect from BS to point
    ...
    if isInLoS
        ...
        loss_outdoor(j(l),k(l),1) = max(Llos,MCL);
...
%disp('Micro - Indoor calculations ');
