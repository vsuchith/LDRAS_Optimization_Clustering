...
% Rect from BS to point
    ...
    if isInLoS
        ...
        loss_outdoor(j(l),k(l)) = max(Llos,MCL);
        %xin(j(l),k(l))=loss_outdoor(j(l),k(l))+p_vue-pl_d-p_cue;
       % xin(j(l),k(l))=gammad_db-loss(j(l),k(l))+pl_d+p_cue;
    else
        ...
        if parallel
            d1 = sqrt((x_p-x_bs)^2 + (y_p-y_bs)^2);
            loss_outdoor(j(l),k(l)) = 40*log10(d1) + 7.8 - 18*log10(htx_eff) - 18*log10(hrx_eff) + 2*log10(freq/1e9)+20;
            %xin(j(l),k(l))=gammad_db-loss(j(l),k(l))+pl_d+p_cue;
           % xin(j(l),k(l))=loss_outdoor(j(l),k(l))+p_vue-pl_d-p_cue;
        else            
            if rx_str * bs_str < 4
...
for l = 1:length(j)    
    ...
    if (~isempty(bs_street_x))
%         if (rx_build_x == bs_street_x) || (rx_build_x == bs_street_x - 1)|| (rx_build_x == bs_street_x + 1)|| (rx_build_x == bs_street_x +2)|| (rx_build_x == bs_street_x +3)|| (rx_build_x == bs_street_x +4)|| (rx_build_x == bs_street_x +5)...
%                 || (rx_build_x == bs_street_x +6)|| (rx_build_x == bs_street_x +7)|| (rx_build_x == bs_street_x +8)|| (rx_build_x == bs_street_x +9)|| (rx_build_x == bs_street_x -2)
            isInLoS = 1;            
    ...
    if (~isempty(bs_street_y))
%         if (rx_build_y == bs_street_y) || (rx_build_y == bs_street_y - 1)|| (rx_build_y == bs_street_y +1)|| (rx_build_y == bs_street_y +2)
            isInLoS = 1;            
    ...
    if isInLoS
        ...
        if num_opt == 1
            ...
            Lout = max(Lout,MCL);      
            PL = Lout;
            loss_indoor(j(l),k(l))=max(PL,MCL);
            %xin(j(l),k(l))=gammad_db-loss(j(l),k(l))+pl_d+p_cue;
        else
            ...
            loss_indoor(j(l),k(l))=max(PL,MCL);
            %xin(j(l),k(l))=gammad_db-loss(j(l),k(l))+pl_d+p_cue;
            %xin(j(l),k(l))=loss_indoor(j(l),k(l))+p_vue-pl_d-p_cue;
        end
...
% a=(((loss_outdoor(:,:))<93.89)&(loss_outdoor(:,:)>0));
% b=(((loss_indoor(:,:))<113)&(loss_indoor(:,:)>0));
% %imagesc(a+b);
% z=a+b;
